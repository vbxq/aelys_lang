...
impl ConstantFolder {
    ...
    pub(super) fn try_fold_and(&mut self, left: &TypedExpr, right: &TypedExpr, original: &TypedExpr) -> Option<TypedExpr> {
        ...
        match &left_val.kind {
            TypedExprKind::Bool(false) => {
                ...
                return Some(TypedExpr::new(TypedExprKind::Bool(false), InferType::Bool, original.span));
            }
    ...
    pub(super) fn try_fold_or(&mut self, left: &TypedExpr, right: &TypedExpr, original: &TypedExpr) -> Option<TypedExpr> {
        let left_folded = self.try_fold(left);
        let left_val = left_folded.as_ref().unwrap_or(left);
        ...
        match &left_val.kind {
            TypedExprKind::Bool(true) => {
                self.stats.constants_folded += 1;
                return Some(TypedExpr::new(TypedExprKind::Bool(true), InferType::Bool, original.span));
            }
            TypedExprKind::Bool(false) => {
                self.stats.constants_folded += 1;
                return Some(self.try_fold(right).unwrap_or_else(|| right.clone()));
