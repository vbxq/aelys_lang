...
fn native_read_char(vm: &mut VM, _args: &[Value]) -> Result<Value, RuntimeError> {
    ...
    let b = match bytes.next() {
        Some(Ok(b)) => b,
        Some(Err(e)) => { eprintln!("io.read_char error: {}", e); return Ok(Value::null()); }
    ...
    let len = if b & 0x80 == 0 { 1 }
        ...
        else { 1 }; // invalid start byte, treat as single

    for i in 1..len {
        if let Some(Ok(c)) = bytes.next() { buf[i] = c; } else { break; }
    }
...
fn native_move_cursor(vm: &mut VM, args: &[Value]) -> Result<Value, RuntimeError> {
    let x = args[0].as_int().ok_or_else(|| vm.runtime_error(
        aelys_common::error::RuntimeErrorKind::TypeError {
            operation: "io.move_cursor", expected: "int", got: vm.value_type_name(args[0]).to_string()
        }))?;
    let y = args[1].as_int().ok_or_else(|| vm.runtime_error(
        aelys_common::error::RuntimeErrorKind::TypeError {
            operation: "io.move_cursor", expected: "int", got: vm.value_type_name(args[1]).to_string()
        }))?;
